...
import { ETenderType, ETenderTypeEn, ITender, ITenderAPI, ITenderByTwoObjectsAPI } from "../model/tender.model";
...
export const TenderAPI = createApi({
    ...
    endpoints: (build) => ({
        ...
        getUserTenders: build.query<ITenderAPI[], {userId: string, type?: ETenderTypeEn}>({
            query: ({userId, type}) => ({
                url: `/GetUserTenders/${userId}` + (type ? `?type=${type}` : '')
            }),
        }),

        deleteTender: build.mutation<void, {tenderId: ITender['id'], type?: ETenderTypeEn}>({
            query: ({tenderId, type}) => ({
        ...
        // TENDER BY TYPE
        getTender: build.query<ITenderAPI, {tenderId: number, type: ETenderTypeEn}>({
            query: ({tenderId, type}) => ({
